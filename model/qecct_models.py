...
# ============================================================
# 1. Toric Code Utilities (legacy) + Surface Code
# ============================================================
...
class ToricCode:
    ...
    def get_syndrome_batch(self, noise: np.ndarray) -> np.ndarray:
        """Vectorized batch syndrome: noise (B, n) -> syndromes (B, n_s)."""
        return (noise @ self.H.T) % 2
...
class SurfaceCode:
    """
    Rotated Surface Code on a d×d lattice.
    Distance d = d.
    Physical qubits: n = d^2.
    Stabilizers: n_s = d^2 - 1.
    """

    def __init__(self, d: int):
        self.L = d  # using L as d for consistency with scripts
        self.d = d
        self.n = d * d
        self.n_s = d * d - 1
    ...
    def _build_parity_check(self):
        d = self.d
        # Rotated d x d surface code: checkerboard face stabilizers + boundary stabilizers.
        # Conventions (matches d=3 explicit hardcoding, generalized to odd d):
        #   - Face (i,j) for i,j in [0, d-2] covers qubits {i*d+j, i*d+j+1, (i+1)*d+j, (i+1)*d+j+1}
        #   - Checkerboard color: (i+j)%2 == 0 -> Z-face, odd -> X-face
        #   - Top edge (row=0)     : Z-boundary on column pairs (1,2), (3,4), ...
        #   - Bottom edge (row=d-1): Z-boundary on column pairs (0,1), (2,3), ...
        #   - Left edge (col=0)    : X-boundary on row pairs (0,1), (2,3), ...
        #   - Right edge (col=d-1) : X-boundary on row pairs (1,2), (3,4), ...
        # This satisfies:
        #   (a) each column of H_z / H_x contains at most 2 ones (pymatching requirement)
        #   (b) CSS condition H_x @ H_z^T = 0 mod 2 (X/Z stabilizers commute)
        ...
        for i in range(d - 1):
            for j in range(d - 1):
                qs = [qi(i, j), qi(i, j + 1), qi(i + 1, j), qi(i + 1, j + 1)]
                if (i + j) % 2 == 0:
                    stabs_z.append(qs)
                else:
                    stabs_x.append(qs)
        ...
        for col in range(1, d - 1, 2):                     # top Z
            stabs_z.append([qi(0, col), qi(0, col + 1)])
        for col in range(0, d - 1, 2):                     # bottom Z
            stabs_z.append([qi(d - 1, col), qi(d - 1, col + 1)])
        for row in range(0, d - 1, 2):                     # left X
            stabs_x.append([qi(row, 0), qi(row + 1, 0)])
        for row in range(1, d - 1, 2):                     # right X
            stabs_x.append([qi(row, d - 1), qi(row + 1, d - 1)])

        n_z = len(stabs_z)
        n_x = len(stabs_x)
        H_z = np.zeros((n_z, self.n), dtype=np.float32)
        H_x = np.zeros((n_x, self.n), dtype=np.float32)
        for r, qs in enumerate(stabs_z):
            for q in qs:
                H_z[r, q] = 1
        for r, qs in enumerate(stabs_x):
            for q in qs:
                H_x[r, q] = 1
    ...
    def _build_logical_operators(self):
        d = self.d
        logical = np.zeros((1, self.n), dtype=np.float32)
        # Logical X: horizontal chain across the top row (generalizes d=3).
        logical[0, list(range(d))] = 1
        return logical

    def _build_mask(self):
        total = self.n + self.n_s
        H_full = self.H
        adj = np.zeros((total, total), dtype=np.float32)
        for j in range(self.n_s):
            for i in range(self.n):
    ...
    def sample_independent_noise(self, p: float, batch_size: int = 1) -> np.ndarray:
        return (np.random.random((batch_size, self.n)) < p).astype(np.float32)

    def sample_depolarization_noise(self, p: float, batch_size: int = 1) -> np.ndarray:
        noise = np.zeros((batch_size, self.n), dtype=np.float32)
        rand = np.random.random((batch_size, self.n))
        x_error = (rand < p / 3).astype(np.float32)
        z_error = ((rand >= p / 3) & (rand < 2 * p / 3)).astype(np.float32)
        y_error = ((rand >= 2 * p / 3) & (rand < p)).astype(np.float32)
        # For surface code, we often treat X and Z decoherence separately or combined.
        # Here we just return the bit-flip (X) component for the decoder to handle.
...
class QECCT(nn.Module):
    ...
    Args:
        code: SurfaceCode or ToricCode instance
    ...
    def __init__(self, code: SurfaceCode, N: int = 6, d_model: int = 128,
    ...
    def forward(self, syndrome: torch.Tensor, return_hidden: bool = False) -> dict:
        """
        Forward pass.

        Args:
            syndrome: (B, n_s) for perfect measurements or (B, T, n_s) for faulty
            return_hidden: if True, include intermediate hidden states

        Returns:
            dict with 'prediction' (soft noise), 'noise_est' (g_ω output),
            and optionally 'hiddens' (list of hidden states per layer)
        """
        if self.use_faulty and syndrome.dim() == 3:
            return self._forward_faulty(syndrome)
        else:
            return self._forward_perfect(syndrome, return_hidden)

    def _forward_perfect(self, syndrome: torch.Tensor,
